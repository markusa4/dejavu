...
bool refinement::refine_coloring(sgraph *g, coloring *c, change_tracker *changes, invariant* I, int init_color_class,
    ...
    while(!cell_todo.empty()) {
        ...
        if(next_color_class_sz == 1 && !(config.CONFIG_IR_DENSE && dense_dense)) {
            ...
            if(dense_dense) { // dense-dense
                if(g->max_degree > 127) {
                    ...
                    comp = comp && refine_color_class_dense_dense127(g, c, next_color_class, next_color_class_sz,
                                                                     &color_class_splits, I);
                }
            } else { // dense-sparse
            ...
            if(g->max_degree > 127) {
                ...
                // version with reduced workspace size
                comp = comp && refine_color_class_sparse127(g, c, next_color_class, next_color_class_sz,
                                        &color_class_splits, I);
            }
        }
        if(!comp)
        ...
        while(!color_class_splits.empty()) {
            ...
            if(track_changes && changes != nullptr)
...
void refinement::assure_initialized(sgraph *g) {
    if(!initialized) {
        ...
        singletons.initialize_from_array(workspace_int + n * 2, n);
        old_color_classes.initialize_from_array(workspace_int + n * 3, n);
        neighbours.initialize_from_array(workspace_int + n * 4, n);
        neighbour_sizes.initialize_from_array(workspace_int + n * 6, n);
        queue_pointer.initialize_from_array(workspace_int + n * 8, n);
        color_vertices_considered.initialize_from_array(workspace_int + n * 10, n);
        scratch = workspace_int + n * 12;
        scratch_set.initialize_from_array(workspace_int + n * 13, n);

        color_class_splits.initialize(n);
        neighbours127.initialize(n);
...
bool refinement::refine_coloring_first(sgraph *g, coloring *c, int init_color_class) {
    ...
    while(!cell_todo.empty()) {
        ...
        if(next_color_class_sz == 1 && !(config.CONFIG_IR_DENSE && dense_dense)) {
            // singleton
            comp = comp && refine_color_class_singleton_first(g, c, next_color_class, next_color_class_sz,
                                                              &color_class_splits);
        } else if(config.CONFIG_IR_DENSE) {
            if(dense_dense) { // dense-dense
                comp = comp && refine_color_class_dense_dense_first(g, c, next_color_class, next_color_class_sz,
                                                                    &color_class_splits);
            } else { // dense-sparse
                comp = comp && refine_color_class_dense_first(g, c, next_color_class, next_color_class_sz,
                                                              &color_class_splits);
            }
        } else { // sparse
            comp = comp && refine_color_class_sparse_first(g, c, next_color_class, next_color_class_sz,
                                                           &color_class_splits);
        }
        if(!comp)
            break;
...
bool refinement::refine_color_class_sparse(sgraph *g, coloring *c, int color_class, int class_size,
...
bool refinement::refine_color_class_sparse127(sgraph *g, coloring *c, const int color_class, const int class_size,
                                              work_list_pair_bool* color_class_split_worklist, invariant* I) {
    // for all vertices of the color class...
    bool comp, mark_as_largest;
    int i, j, cc, end_cc, largest_color_class_size, acc_in, singleton_inv1, singleton_inv2, acc, total;
    ...
    while(!old_color_classes.empty()) {
        ...
        while (j < vcount + 1) {
            ...
            if((neighbours127.get(v) == -1))
                continue;
            const int v_new_color = _col + _col_sz - neighbour_sizes.get(neighbours127.get(v) + 1);
            neighbours127.set(v, -1);
            if(v_new_color == _col)
                continue;
            const int vertex_old_pos = c->vertex_to_lab[v];
...
bool refinement::refine_color_class_sparse_first(sgraph *g, coloring *c, int color_class, int class_size,
...
bool refinement::refine_color_class_singleton(sgraph *g, coloring *c, int color_class, int class_size,
                                              work_list_pair_bool* color_class_split_worklist, invariant* I) {
    bool comp;
...
bool refinement::refine_color_class_dense(sgraph *g, coloring *c, int color_class, int class_size,
                                          work_list_pair_bool* color_class_split_worklist, invariant* I) {
    bool comp;
...
bool refinement::refine_color_class_dense_dense(sgraph *g, coloring *c, int color_class, int class_size,
                                                work_list_pair_bool* color_class_split_worklist, invariant* I) {
    bool comp;
...
bool refinement::refine_color_class_dense_dense127(sgraph *g, coloring *c, int color_class, int class_size,
                                                   work_list_pair_bool* color_class_split_worklist, invariant* I) {
    bool comp;
    int i, j, acc, cc, largest_color_class_size;
    cc = color_class; // iterate over color class
    ...
    // in dense-dense we dont sort, instead we just iterate over all cells (which are already sorted)
    // for every cell...
    int singleton_inv1 = 0;
    int singleton_inv2 = 0;
    for(j = 0; j < g->v_size;) {
        const int col    = j;
        j     += c->ptn[j] + 1;
        const int col_sz = c->ptn[col] + 1;
...
bool refinement::refine_color_class_singleton_first(sgraph *g, coloring *c, int color_class, int class_size,
                                                    work_list_pair_bool* color_class_split_worklist) {
    bool comp;
...
bool refinement::refine_color_class_dense_first(sgraph *g, coloring *c, int color_class, int class_size,
                                                work_list_pair_bool* color_class_split_worklist) {
...
bool refinement::refine_color_class_dense_dense_first(sgraph *g, coloring *c, int color_class, int class_size,
                                                      work_list_pair_bool* color_class_split_worklist) {
    // for all vertices of the color class...
...
void work_set_int::initialize_from_array(int* arr, int size) {
    s = arr;
...
void work_list::initialize_from_array(int* arr, int size) {
    this->arr = arr;
    arr_sz    = size;
    init      = false;
    cur_pos   = 0;
